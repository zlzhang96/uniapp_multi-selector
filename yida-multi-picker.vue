  ...
  <view class="uni-select-cy" :style="{ 'z-index': zindex }">
    ...
    >
      ...
      <view class="uni-select-multiple" v-show="realValue.length">
        <!-- Use realValueName instead of realValue -->
        <view
          class="uni-select-multiple-item"
          v-for="(item, index) in realValueName"
          :key="index"
        >
          <view class="uni-select-multiple-item-row">
            <!-- This line controls what to show in the selected line -->
            {{ item }}
          </view>
...
export default {
  ...
  data() {
    return {
      ...
      changevalue: [], //搜索框同步
      realValue: [],
      /* Array to store the selected labels */
      realValueName: [],
    }
  },
  mounted() {
  ...
  methods: {
    ...
    init() {
      if (this.value.length > 0) {
        var items = JSON.parse(JSON.stringify(this.options))

        /* The changevalue doesn't fucking work at all! */
        ...
        /* 手动添加对应的label */
        items.forEach((item) => {
          var itemValue = item[this.svalue]
          let index = this.realValue.indexOf(itemValue)
          if (index > -1) {
            this.realValueName.push(item[this.slabel])
          }
        })
      } else {
        this.changevalue = []
    ...
    handleRemove(index) {
      if (index === null) {
        ...
        this.realValueName.splice(index, 1)
      }
      this.$emit("change", this.changevalue, this.realValue)
    },
    //点击组件列
    handleChange(index, Vue_item) {
      var item = JSON.parse(JSON.stringify(Vue_item))
      let arrIndex = this.realValue.indexOf(item[this.svalue])
      if (arrIndex > -1) {
        this.changevalue.splice(arrIndex, 1)
        this.realValue.splice(arrIndex, 1)
        this.realValueName.splice(arrIndex, 1)
      } else {
        this.changevalue.push(Vue_item)
        this.realValue.push(Vue_item[this.svalue])
        this.realValueName.push(Vue_item[this.slabel])
      }
      this.$emit("change", this.changevalue, this.realValue)
    },
  },
